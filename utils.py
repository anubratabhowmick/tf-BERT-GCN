...
def load_data(dataset, column, frequency, seq_len, year, bert_column, flag):
    ...
    if flag == 'bert_gcn':
        bert_column.append('source_author') # Added by Anubrata
...
#Anubrata Author Data Start
def get_gcn_author_data(train_df, test_df, save_dir, filename):
    lb_dr = os.path.join(save_dir, filename.split('_')[0], filename)
    with open(lb_dr, 'rb') as f:
        embedding = pickle.load(f)
        node2id = pickle.load(f)
    ...
    # Get author list
    author_train = train_df['source_author']
    author_test = test_df['source_author']
    
    # Initialize gcn
    gcn_author_train = np.zeros(shape=(1, 1))
    gcn_author_test = np.zeros(shape=(1, 1))

    # Append GCN values to train and test author
    for eachElem in author_train:
        key = eachElem.split(';')[-1]
        gcn_author_train = np.append(gcn_author_train, [[node2id[key]]], axis=0)


    for eachElem in author_test:
        key = eachElem.split(';')[-1]
        gcn_author_test = np.append(gcn_author_test, [[node2id[key]]], axis=0)

    return gcn_author_train, gcn_author_test, embedding, node2id
#Anubrata Author Data End
